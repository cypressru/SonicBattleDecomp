...
extern struct UnknownPoolNode404ec *gUnknown_0300547c;
...
struct UnknownGraphicsSources47448 {
    const void *reserved;
...
void FUN_080470e4(struct UnknownState482d0 *state) {
    ...
    {
        struct UnknownPoolNode404ec *node =
            (struct UnknownPoolNode404ec *)((u32)nodes + order->indices[i] * stride);

        node->data = gUnknown_08edda68[gUnknown_03000274[state->field20 + 1]];
        node->field40 = gUnknown_03000274[state->field20 + 2];
        node->field30 = node->field10;
        node->field34 = node->field14;
        node->field26 = 0;
    }
...
void FUN_080471dc(struct UnknownState482d0 *state) {
    ...
    state->field40--;
    if (state->field40 == 0) {
        state->field10 = state->field30;
        state->field12.half.high = state->field34;
        state->callback = (const void *)((u32)FUN_08047fe4 + 1);
        gUnknown_03005440.field8 &= ~4;
    }
    FUN_080405f4(state->field2, state->field39);
}

void FUN_08047258(struct UnknownState482d0 *state) {
    s16 index;
    s16 nextIndex;
    s32 offset;
    struct UnknownGraphicsRecord47258 *graphics;

    state->field26 += 7;
    gUnknown_030002d0.first = FUN_0804a59c(0x400000, FUN_08040698(state->field26));
    gUnknown_030002d0.second = 256;
    *(volatile u16 *)&gUnknown_030002d0.third = 0;
    FUN_0804a5c4(&gUnknown_030002d0, gUnknown_030044d6, 1, 8);
    state->field22 = 1;

    index = (s8)(state->field26 - 65) >= 0 ? 4 : 0;
    graphics = &gUnknown_03000290[index];
    state->graphics = graphics;
    offset = FUN_0804a59c(FUN_08040698(state->field26) << 4, 0x4000);
    index++;
    nextIndex = index + 1;
    graphics->offset = gUnknown_03000290[index].offset = -(offset + 32);
    gUnknown_03000290[nextIndex].offset = gUnknown_03000290[nextIndex + 1].offset =
        FUN_0804a59c(FUN_08040698(state->field26) << 5, 0x4000) - 16;

    if ((state->field3 & 4) != 0) {
        if ((s8)state->field26 >= 0) {
            state->field3 &= ~4;
...
void FUN_08047384(struct UnknownState482d0 *state) {
    s16 i;
    struct UnknownPoolNode404ec *nodes;
    struct UnknownPoolOrder47384 *order;
    ...
    for (i = 0; i < gUnknown_03000278.count; i++) {
        ...
        order = &gUnknown_03000278;
        nodes = gUnknown_0300547c;
        node = (struct UnknownPoolNode404ec *)(order->indices[i] * sizeof(*node) + (u32)nodes);

        if (node->field36 == state->field20) {
            break;
        }
    }
    if (i >= gUnknown_03000278.count) {
        gUnknown_03005440.field8 &= ~4;
        FUN_0804051c(state);
        return;
    }
    {
        struct UnknownPoolNode404ec *node = &nodes[order->indices[i]];
        u8 flags;

        node->data = (const void *)((u32)FUN_08047258 + 1);
        flags = node->field3 & 4;
        if (flags != 0) {
            flags = 0x80;
        }
    ...
    gUnknown_030044d0[3].fourth = 256;
    FUN_08045a5c((u8)state->field20);
    FUN_0804051c(state);
}

void FUN_08047448(struct UnknownState482d0 *state) {
    u32 oldProgress = state->field26;
    s16 index;
    s16 nextIndex;
    s32 offset;
    struct UnknownGraphicsRecord47258 *graphics;

    state->field26 = oldProgress + 8;
    if ((u8)state->field26 == 64 || (u8)state->field26 == 192) {
        const void *source;

        state->field26 = oldProgress + 136;
        source = gUnknown_081b94c8[state->field36 - 1].sources[state->field37];
        FUN_0804a5b8(source, (void *)(0x02016A00 + state->field38 * 0x3600));
        FUN_0804033c((void *)(0x02016A00 + state->field38 * 0x3600),
                     (void *)(0x06010000 + state->field20 * 32), 0x1200);
        return;
    }
    ...
    if ((state->field3 & 4) != 0) {
        if ((u8)state->field26 == 128) {
            state->graphics = gUnknown_081a7f38;
            state->field22 = 0;
            state->callback = (const void *)((u32)FUN_08047fe4 + 1);
            gUnknown_03005440.field8 &= ~4;
        }
    } else if ((u8)state->field26 == 0) {
        state->graphics = gUnknown_081a7f18;
        state->field22 = 0;
        state->callback = (const void *)((u32)FUN_08047fe4 + 1);
        gUnknown_03005440.field8 &= ~4;
...
void FUN_080476e0(void) {
    s16 x;
    s16 y;
    s16 i;

    FUN_0801fba0(16, -gUnknown_03000288.first);
    FUN_0801fba0(18, -gUnknown_03000288.second);
    x = gUnknown_03005494 - gUnknown_03000288.first;
    y = gUnknown_03005490 - gUnknown_03000288.second;
    if (x < 0) {
        x = 0;
    } else if (x > gUnknown_081a2a4c[gUnknown_03005440.field31].width - 240) {
        x = gUnknown_081a2a4c[gUnknown_03005440.field31].width - 240;
    }
    if (y < 0) {
        y = 0;
    } else if (y > gUnknown_081a2a4c[gUnknown_03005440.field31].height - 160) {
        y = gUnknown_081a2a4c[gUnknown_03005440.field31].height - 160;
    }
    for (i = 0; i <= 2; i++) {
        FUN_0801fba0(i * 4 + 20, x);
        FUN_0801fba0(i * 4 + 22, y);
    }
}

void FUN_080477c0(struct UnknownState482d0 *state) {
    ...
    case 1:
        gUnknown_03000288.second = 0;
        gUnknown_03000288.first = 0;
        FUN_080476e0();
        state->field27++;
        break;
    case 2:
        FUN_0804051c(state);
        gUnknown_03005440.field8 &= ~0x14;
        break;
    }
}

void FUN_0804785c(struct UnknownState482d0 *state) {
    u32 progress = state->field26 + 1;

    state->field26 = progress;
    if (gUnknown_03005440.field45 != 0) {
        FUN_0803fe98(0, 16, (u8)progress >> 1);
        FUN_0803fe98(192, 320, (u8)state->field26 >> 1);
    } else {
        FUN_0803fe98(0, 512, (u8)progress >> 1);
    }
    if (state->field26 > 31) {
        gUnknown_03005440.field8 = (gUnknown_03005440.field8 & ~4) | 8;
        FUN_0804051c(state);
    }
}

void FUN_080478c8(struct UnknownState482d0 *state) {
    u32 progress = state->field26 + 1;

    state->field26 = progress;
    if (gUnknown_03005440.field45 != 0) {
        u16 fade = 16 - ((u8)progress >> 1);

        FUN_0803fe98(0, 16, fade);
        FUN_0803fe98(192, 320, 16 - ((u8)state->field26 >> 1));
    } else {
        FUN_0803fe98(0, 512, 16 - ((u8)progress >> 1));
    }
    if (state->field26 > 31) {
        gUnknown_03005440.field8 &= ~0xC;
        FUN_0804051c(state);
    }
}

void FUN_08047948(struct UnknownState482d0 *state) {
    u32 progress = state->field26 + 1;
    u16 packed;
    u16 intensity;
    volatile u16 color;
    ...
    state->field26 = progress;
    if (gUnknown_03005440.field45 != 0) {
        FUN_0803fd9c(0, 16, (u8)progress >> 1);
        FUN_0803fd9c(192, 320, (u8)state->field26 >> 1);
...
void FUN_08047a00(struct UnknownState482d0 *state) {
    u32 progress = state->field26 + 1;
    ...
    if (gUnknown_03005440.field45 != 0) {
        ...
        u16 intensity;
        u16 color;
        volatile u16 *colorDestination;

        FUN_0803fd9c(0, 16, 16 - ((u8)progress >> 1));
        FUN_0803fd9c(192, 320, 16 - ((u8)state->field26 >> 1));
        intensity = (u16)(32 - state->field26);
        packed = (intensity << 10) | (intensity << 5) | intensity;
...
void FUN_08047acc(struct UnknownState482d0 *state) {
    state->field27 = state->field26;
    if ((u8)state->field27 > 78) {
        state->field27 = 78;
    }
    state->field12.half.high = 56 - FUN_0804a59c(FUN_08040684(state->field27), 512);
    state->field20 = state->field27 & 0xF0;
    if (state->field20 > 48) {
        state->field20 = 48;
    }
    state->field26 += 4;
    if ((u8)state->field26 > 248) {
        gUnknown_03005440.field8 &= ~4;
        FUN_0804051c(state);
    } else {
...
void FUN_08047c30(struct UnknownState482d0 *state) {
...

    {
        ...
        if ((u8)(frame & mask) > 4) {
            state->field26 = zero;
            nextFrame = state->field27 + 1;
            state->field27 = nextFrame;
            nextFrame &= mask;
            if (nextFrame > 5) {
                state->field27 = zero;
            }
...
void FUN_08047d1c(struct UnknownState482d0 *state) {
    struct UnknownPoolOrder47384 *order;
    struct UnknownPoolNode404ec *nodes;
    s16 i;
    s16 identity;
    ...
    if (gUnknown_03000274[state->field20 + 1] > 6 ||
        (gUnknown_03000274[state->field20] != 0 && i >= gUnknown_03000278.count)) {
        gUnknown_03005440.field8 &= ~4;
        FUN_0804051c(state);
        return;
    }
    ...
    if ((u8)(variant - 4) <= 1) {
        state->graphics = gUnknown_081a7f80;
    } else {
        FUN_0804033c((void *)0x02028000, (void *)0x06013200, gUnknown_081bbbe8[variant].size);
        state->graphics = gUnknown_081a7f78;
    }
    if (gUnknown_03000274[state->field20] == 0) {
        state->field10 = 120;
    } else {
        state->field10 = gUnknown_0300547c[gUnknown_03000278.indices[i]].field10;
    }
    state->field24 = 0x84;
    state->callback = gUnknown_08edda6c[(u8)variant];
...
void FUN_08048014(struct UnknownState482d0 *state) {
    switch (state->field27) {
    case 0:
        state->field30 = 0x131;
        state->field27++;
    case 1:
        *(s32 *)&state->field8 -=
            FUN_0804a59c(*(s32 *)&state->field8 - *(s32 *)&state->filler28, 8);
        if ((s16)state->field10 > 0x11F) {
            FUN_08046918(state);
            FUN_0804051c(state);
            return;
        }
...
void FUN_080480d4(struct UnknownState482d0 *state) {
    switch ((state->field27 >> 1) & 3) {
    case 0:
        FUN_0803fd9c(16, 176, 16);
        break;
    case 1:
    case 3:
        FUN_0803fe98(16, 176, 0);
        break;
    case 2:
        FUN_0803fe98(16, 176, 16);
        break;
    }
    state->field27++;
    if ((u8)state->field27 > 31) {
        gUnknown_03005440.field8 &= ~4;
        FUN_0804051c(state);
    }
}
...
void FUN_08048200(struct UnknownState482d0 *state) {
    s32 target = (s16)state->field30;

    if ((state->field3 & 2) != 0) {
        target++;
    }
    target <<= 16;
    target = FUN_0804a59c(*(s32 *)&state->field8 - target, 8);
    *(s32 *)&state->field8 -= target;
    if ((s16)state->field30 == (s16)state->field10) {
        state->field8 = 0;
        state->field3 &= 0x7D;
        state->callback = (const void *)((u32)FUN_08047fe4 + 1);
    }
...
void FUN_0804825c(struct UnknownState482d0 *state) {
    volatile struct UnknownPair4825c *pair = &gUnknown_03000288;
    u16 first = pair->first;
    u16 second;

    first += state->field30;
    state->field10 = first;
    second = pair->second;
    second += state->field34;
