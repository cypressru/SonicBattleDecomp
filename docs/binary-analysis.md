...
and blend helpers were initially reconstructed in C. They now use the requested C++ fallback:
original language is not established by that earlier C byte match. Objdiff verifies all 1,734
instruction bytes, all 290 owned non-code bytes, and all 33 functions at 100%. The complete
independently linked comparison and C++ emission adjustment are documented in `display-tu-cpp.md`.
...
inline literal island and a trailing pool, terminates with a normal return, and ends exactly at the
independently accepted `FUN_0801e3cc` entry. Clean C reproduces every owned instruction and
...
The gap scan also recovered two consecutive unreferenced functions at `0x0801D370` and
`0x0801D3AC`. Their complete 0x36- and 0x50-byte Thumb bodies, alignment padding, and trailing
literal pools occupy the full gap after `FUN_0801d288` and end exactly at the independently
accepted `FUN_0801d408` entry. The first clears two flag groups in the established 24-byte entity
record. The second follows the independently established entity/metadata coordinate lookup shape
and preserves the retail program's two consecutive `ArcTan2` calls before indexing its own angle
table. Clean C reproduces all 0x86 instruction bytes and relocations. No decoded calls or stored
pointers were found, so both retain generic address names and
`recovered-unreferenced+exact-body` provenance.

The gap immediately after `FUN_0801e9ec` contains another complete unreferenced function at
`0x0801EA38`. Its 0x2C-byte Thumb body and two-word trailing literal pool end exactly at the
independently accepted `FUN_0801ea6c` entry. When the established entity-state predicate returns
zero, it installs the mapped common callback and copies the result of `FUN_0801d068` into the
entity's linked-index byte, matching the independently reconstructed `FUN_0801f030` field shape.
Clean C reproduces every instruction and relocation. No decoded call or stored pointer was found,
so the generic address name and `recovered-unreferenced+exact-body` provenance record only the
supported boundary and behavior.

The gap before `FUN_08023038` contains a complete unreferenced sibling at `0x08023010`. Its
0x1E-byte Thumb body, alignment halfword, and two-word trailing pool end exactly at the independently
accepted callback. Both functions test the same 512-halfword palette range through
`FUN_0802067c`; the recovered sibling installs `FUN_0803d748` and returns without the additional
transition call made by `FUN_08023038`. Clean C reproduces every instruction and relocation. No
decoded call or stored pointer was found, so it retains the generic address name and
`recovered-unreferenced+exact-body` provenance.

The gap before `FUN_08024b90` contains a complete 0xE-byte unreferenced function at
`0x08024B80`. It loads the established list-node byte at offset 6 and passes it with zero to
`FUN_0801fed8`, making it a byte-identical sibling of the independently referenced
`FUN_080244ac`, `FUN_080245dc`, and `FUN_08024c20`. Its normal prologue/return and alignment
...
The gap before `FUN_0802d76c` contains another complete 0xE-byte unreferenced cleanup wrapper at
`0x0802D75C`. It loads the established list-node byte at offset 6 and passes it with zero to
`FUN_0801fed8`, matching the four independently bounded wrapper siblings already reconstructed.
Its normal prologue/return and alignment halfword end exactly at the next accepted callback. Clean
C reproduces every instruction and the call relocation. No decoded reference was found, so it
retains the generic address name and `recovered-unreferenced+exact-body` provenance.

The gap before `FUN_08039c3c` contains a complete unreferenced halfword-stream helper at
`0x08039C1C`. It copies source halfwords up to, but not including, the `0xFFFE` terminator and
returns the advanced destination pointer. The preserved `r0` value and LR pop through `r1`
independently establish that pointer return rather than a guessed `void` signature. Its complete
0x1A-byte body, alignment halfword, and trailing sentinel literal end exactly at the next accepted
entry. Clean C reproduces every instruction and mapping transition. No decoded reference was found,
so it retains the generic address name and `recovered-unreferenced+exact-body` provenance.

The gap before `FUN_0803b69c` contains three complete unreferenced functions at `0x0803B63C`,
`0x0803B694`, and `0x0803B698`. The 0x4E-byte first body clears a 24-by-16-word strided region
selected by its byte argument; `0xFF` instead selects a bulk zero-fill of `0x06007A00`. Its inline
destination literal, alternate-path pool, complete return, and call relocation establish the full
body. The latter two are separately aligned `bx lr` leaf bodies, each followed by compiler padding;
compiling two empty C functions reproduces both independent two-byte extents and their spacing
before the accepted dispatcher. All 0x52 instruction bytes and mapping transitions match. No
decoded reference was found for any of the three, so their generic address names and
`recovered-unreferenced+exact-body` provenance remain conservative.

The gap before `FUN_080382bc` contains a complete unreferenced 0x4E-byte sibling of the strided
clear helper at `0x08038264`. It has the same 24-by-16-word clear and `0xFF` bulk-zero path, but its
per-row stride is 576 bytes rather than 640. The distinct stride constant, inline destination
literal, alternate-path pool, complete return, and call relocation establish a separate function
ending exactly at the next accepted entry. Clean C reproduces every instruction, mapping transition,
and relocation. No decoded reference was found, so it retains the generic address name and
`recovered-unreferenced+exact-body` provenance.
